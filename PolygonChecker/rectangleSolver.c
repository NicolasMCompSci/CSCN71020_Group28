...
bool checkIfRectangleByPoints(POINT pointOne, POINT pointTwo, POINT pointThree, POINT pointFour)
{
	...
	if (pointOne.x != pointTwo.x || pointTwo.y != pointThree.y || pointThree.x != pointFour.x || pointFour.y != pointOne.y || pointTwo.y == 0 || pointThree.x == 0 || pointThree.y == 0 || pointFour.x == 0)
		return false;
	else
		return true;
}
